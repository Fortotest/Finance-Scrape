...
async function testTV() {
  ...
  try {
    ...
    // Check js-symbol-change and js-symbol-change-pt
    const change = await page.$eval('.js-symbol-change', el => el.textContent).catch(() => null);
    const changePer = await page.$eval('.js-symbol-change-pt', el => el.textContent).catch(() => null);
    
    console.log({ price, change, changePer });
